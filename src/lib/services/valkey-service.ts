 ...
 */
export async function initValKeyClient(config: ValKeyConfig = DEFAULT_CONFIG): Promise<void> {
	try {
		...
		const url = `redis://${config.username ? `${config.username}:${config.password}@` : ''}${config.host}:${config.port}`;

		client = createClient({
			url,
			socket: {
				reconnectStrategy: (retries: number): number => Math.min(retries * 50, 1000)
			}
		});
 ...
 */
export async function getKeysValues(pattern: string): Promise<Record<string, string>> {
	...
	try {
		...
		console.error(`Error retrieving keys matching ${pattern} from Valkey:`, error);
		return {};
	}
}
