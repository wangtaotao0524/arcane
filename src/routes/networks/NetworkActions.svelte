<script lang="ts">
  ...
  const isDefaultNetwork = $derived(
    name === "host" || name === "bridge" || name === "none"
  );
...
{#if id}
  ...
  <Button
    ...
    title={isDefaultNetwork
      ? "Cannot delete default network"
      : "Delete Network"}
    disabled={isDeleting || isDefaultNetwork}
  >
    ...
    <Dialog.Content>
      <Dialog.Header>
        ...
        <Dialog.Description>
          ...
          undone. Networks currently in use by containers cannot be deleted.
        </Dialog.Description>
      </Dialog.Header>
