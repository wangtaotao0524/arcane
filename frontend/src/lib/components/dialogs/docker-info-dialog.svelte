<script lang="ts">
	...
	function formatMemory(bytes_val: number): string {
		return bytes.format(bytes_val) ?? '-';
	}
</script>
				...
				<div class="bg-muted/50 space-y-1.5 rounded-lg p-3">
					...
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center gap-2">
							<span class="font-mono text-sm">{dockerInfo?.gitCommit?.slice(0, 8) ?? '-'}</span>
							{#if dockerInfo?.gitCommit}
								<Button variant="ghost" size="icon" class="size-6" onclick={() => copy(dockerInfo?.gitCommit)}>
			...
			<div>
				...
				<div class="bg-muted/50 space-y-1.5 rounded-lg p-3">
					...
					<div class="flex items-center justify-between">
						...
						<Badge variant="secondary">{formatMemory(dockerInfo?.memTotal ?? 0)}</Badge>
					</div>
				</div>
			</div>
