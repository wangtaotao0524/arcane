	...
	<div class="w-full max-w-md space-y-8">
		<div class="text-center">
			{#if isRedirecting && !error}
				...
				<p class="mt-2 text-sm">Please wait while we redirect you to your identity provider.</p>
			{:else if error}
				<div class="text-red-600">
					...
					<h2 class="mt-6 text-2xl font-bold">Login Error</h2>
					<p class="mt-2 text-sm">{error}</p>
					<p class="mt-4 text-xs">Redirecting you back to the login page...</p>
				</div>
			{/if}
		</div>
	</div>
