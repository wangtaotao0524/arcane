<script lang="ts">
	...
	onMount(async () => {
		try {
			...
			if (!authUrl) {
				error = 'Unable to generate authentication URL. Please try again or contact your administrator.';
			...
			if (err.message?.includes('discovery')) {
				...
				userMessage = 'Network error connecting to authentication provider. Please check your connection and try again.';
			}
	...
	<div class="w-full max-w-md space-y-8">
		<div class="text-center">
			{#if isRedirecting && !error}
				...
				<p class="text-muted-foreground mt-2 text-sm">Please wait while we redirect you to your identity provider.</p>
			{:else if error}
				<div class="text-destructive">
					...
					<p class="text-muted-foreground mt-4 text-xs">Redirecting you back to the login page...</p>
				</div>
			{/if}
		</div>
	</div>
